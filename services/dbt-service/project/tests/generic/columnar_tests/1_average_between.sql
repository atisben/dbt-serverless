{% test average_between(model, column_name, min_value=None, max_value=None) %}
...
(
    SELECT
        ...
        'average_between' AS test_name,
        'average value of the column should be between a specified range' AS test_rule,
        'min_value = {{min_value}}, max_value = {{max_value}}' AS test_params,
        CAST({{ min_value }} AS NUMERIC) AS min_value ,
        CAST({{ max_value }} AS NUMERIC) AS max_value ,
        CAST((SELECT AVG({{ column_name }}) FROM {{ model }}) AS NUMERIC) AS result
)
{% endtest %}