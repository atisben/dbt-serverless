{% test sum_between(model, column_name, min_value=None, max_value=None) %}
...
(
    SELECT
        ...
        'sum_between' AS test_name,
        'sum of the column should be between a specified range' AS test_rule,
        ...
        CAST((SELECT SUM({{ column_name }}) FROM {{ model }}) AS NUMERIC) AS result
)
{% endtest %}